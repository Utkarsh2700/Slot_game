  ...
  useEffect(() => {
    ...
    const animation = container.animate(
      [
        ...
        { top: `calc(${Math.floor(factor) * 10} / 3 * -100% - ${Math.floor(factor) * 10 * 3}px)`, filter: 'blur(0)' },
      ],
      {
    ...
    const renderSymbols = () => {
      ...
      for (let i = 3; i < 3 + Math.floor(factor) * 10; i++) {
        const icon = new Symbol(i >= 10 * Math.floor(factor) - 2 ? nextSymbols[i - Math.floor(factor) * 10] : Symbol.random());
    ...
    const spin = () => {
      const animationPromise = new Promise((resolve) => (animation.onfinish = resolve));
      const timeoutPromise = new Promise((resolve) => setTimeout(resolve, factor * 1000));
